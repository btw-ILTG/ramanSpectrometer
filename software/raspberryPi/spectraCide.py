...
class mainWindowClass(QtWidgets.QMainWindow, form_class):
	...
	def getSpectra(self):
		with serial.Serial(self.serialPorts[self.serialCombo.currentIndex()], 921600) as mcu, open(fname,fmode) as outf: 		# setup serial for communication to Nucelo and open the data file
			mcu.write(b'r') 																									# send the command to the nucleo to start the capture
...
#		plotSpectra()
	...
	def setSensitivity(self):
		mcu = serial.Serial(self.serialPorts[self.serialCombo.currentIndex()], 921600)
